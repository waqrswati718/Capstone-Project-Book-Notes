...
app.post("/add", async (req, res) => {
  ...
  try {
    await db.query(
      "INSERT INTO books(title,isbn,description,rating) VALUES ($1,$2,$3,$4)",
    ...
    res
      .status(500)
      .send("The book already exists. Update/Edit option can be utilized.");
  }
});
...
app.post("/update", async (req, res) => {
  const { updatedTitle, updatedIsbn, updatedDescription, updatedRating } =
    req.body;
  ...
  try {
    await db.query(
      "UPDATE books SET title = $1, description = $2, rating = $3 WHERE isbn = $4",
